...
def play_game(game_no, player1, player2)
    ...
    i = 1
    while true
        if player1_history.include?(player1.join(',')) && player2_history.include?(player2.join(',')) && player1_history.index(player1.join) == player2_history.index(player2.join)
            # puts "SAFETY NET"
            result = 'player 1'
            break
        else
            ...
            player1 = player1.reverse!
            player2 = player2.reverse!
